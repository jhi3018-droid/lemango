...
## 🔐 POS 권한 방침 / POS Permission Policy (POS 전 단계 필수 준수)

**소유주 확정 방침. POS/매장 서브시스템의 모든 단계(1c 이후 전부)가 이 방침을 반드시 따른다.**
원칙: **조회(보기)는 전 직원 개방 · 작업(등록/취소/차감)은 본인 매장 직원만 · 관리자(grade≥3)는 전 매장 전권.**

### 권한 요약표
| 기능 | 권한 |
|------|------|
| 매출 조회 (sales/revenue viewing) | **전 직원** (매장 무관 — 회사 매출이므로 전원 공유) |
| 매장별 재고 조회 (per-store stock viewing) | **전 직원** (회사 공용 데이터) |
| POS 판매 등록 (sale registration) | **본인 매장 직원만** (사무직/미배정은 판매 등록 불가) |
| 판매 취소/환불 (void/refund) | **본인 매장 직원만** + 🔴 **로그 + 사유 필수** |
| 강제 재고 차감 (forced/manual stock adjustment, 불량/오류) | **본인 매장 직원만** + 🔴 **로그 + 사유 필수** |
| 관리자 (grade≥3) | **위 전부 + 모든 매장** (조회·등록·취소·환불·차감 전권) |

### 🔴 감사 로그 (민감 작업 필수 — 가장 중요)
**판매 취소(void) · 환불(refund) · 강제 재고 차감(forced stock adjustment)** 은 **반드시 기록**한다:
- **누가(WHO) · 언제(WHEN) · 무엇을(WHAT 변경 내용) · 사유(REASON/사유)** 4가지를 모두 기록.
- **사유(REASON)는 필수 입력** — 사유 없이는 작업 완료 불가 (빈 사유 → 차단).
- 기록 방식: 기존 `logActivity(action, target, detail)` (activity-log.js) 감사 추적 + 해당 취소/환불/차감을 **자체 레코드로도 저장**(storeSales soft-void 설계 + 재고조정 로그, 사유 필드 포함).
- 목적: 사후 정산·추적 ("이 재고가 왜 사라졌는가?") — 이 로그가 없으면 재고 불일치를 추적할 수 없음.

### 향후 단계 영향
- **1f (매장별 재고현황 뷰)**: 조회이므로 **전 직원 접근 개방**.
- **Phase 3 (POS 판매 등록)**: **본인 매장 직원만** (관리자는 전 매장).
- **Phase 4 (판매 취소/환불)**: **본인 매장 직원만 + 로그 + 사유 필수**.
- **강제 재고 차감**: **본인 매장 직원만 + 로그 + 사유 필수**.
- ⚠️ **1c 설계 가정 수정**: 초기 1c 설계는 "미배정(사무직) 사용자는 매장 탭에서 빈 화면만" 이었으나, **조회가 전 직원 개방**이므로 매장 탭의 **조회 화면(매출·재고)은 전원 접근 가능**하게, **작업 화면(판매·취소·차감)만 본인 매장 직원+관리자로 게이트**한다. (지금 구현 금지 — 방침만 기록. 1c/1f에서 반영)

> 소유주 메모: 취소/환불/차감 화면을 만들 때 세부 UI/흐름은 추가로 지시 예정 — 단, 위 권한·로그·사유 방침은 **확정(불변)**.
